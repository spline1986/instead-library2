...
room {
   ...
   obj = {
	  ...
	  obj {
		 ...
		 act = function(s)
			if live 'первый сервитор' and disabled 'за ангаром' then
			   enable 'за ангаром'
			end
...
room {
   ...
   dsc = function(s)
	  if not s.seen then
		 s.seen = true
		 return 'Я аккуратно спрыгнул с забора на землю. Леонид спрыгнул следом.^^Местность здесь достаточно пустынна.'
	  else
...
room {
   nam = 'смерть 1',
   disp = '. . .',
   decor = '{@ restore|А может всё было иначе?}',
}
